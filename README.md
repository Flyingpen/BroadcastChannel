...
| ------------ | -------------------------------------------- | -------- | ------ | -------- | ----------------------- | -------------- |
| \*_静态部�?_ | GitHub Pages<br/>EdgeOne Pages<br/>阿里�?OSS | `Astro`  | `./`   | `dist`   | `pnpm run build:static` | `pnpm install` |
...
> **适用平台**：GitHub Pages、EdgeOne Pages、Gitee Pages、阿里云 OSS、腾讯云 COS 等静态托管平�?

#### GitHub Pages 部署

1. **Fork 项目**到你�?GitHub 账号
2. **配置环境变量**�?
...
> 适用于需要在 EdgeOne 平台上以静态资源形式托管站点的场景。
...
| 输出目录 | `dist/edgeone`                      |
| 安装命令 | `pnpm install`                      |
| 编译命令 | `SERVER_ADAPTER=edgeone pnpm build` |

部署步骤：

1. 在 EdgeOne 控制台创建站点时，框架预设选择 **Astro**，根目录保持默认的 `.`。
2. 在“安装命令”填写 `pnpm install`，安装依赖。
3. 在“构建命令”填写 `SERVER_ADAPTER=edgeone pnpm build`，该命令会自动为 EdgeOne 生成静态内容。
4. 构建完成后，EdgeOne 会自动读取 `dist/edgeone` 目录并部署其中的静态资源。
